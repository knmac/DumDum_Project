...
import fr.eurecom.allmenus.CongratBox;
import fr.eurecom.allmenus.LoadMenu;
import fr.eurecom.allmenus.MainMenu;
import fr.eurecom.allmenus.MssgBox;
import fr.eurecom.allmenus.MultiplayerMenu;
import fr.eurecom.allmenus.PauseMenu;
import fr.eurecom.allmenus.SettingMenu;
...
public class GameManager {
	...
	// --------------------------------------------------------------------------
		...
		public enum GameState {
			MAIN_MENU(0), LOAD_MENU(1), MULTIPLAYER_MENU(2), SHOP_MENU(3), SETTING_MENU(4), PAUSE_MENU(5), MSSG_BOX(6), CONGRAT_BOX(7), GAME(8);
		...
		public static void createCorrespondingMenu(GameState menuState) {
			...
			if (menuList[menuState.getValue()] == null) {
				switch (menuState) {
					case MAIN_MENU:
						menuList[menuState.getValue()] = new MainMenu(new DynamicBitmap(
									Parameters.bmpBkMainMenu, new Point(0, 0), 0, screenSize.x, screenSize.y));
						break;
					case LOAD_MENU:
						try {
							menuList[menuState.getValue()] = new LoadMenu(new DynamicBitmap(
									Parameters.bmpBkSubMenu, new Point(0, 0), 0, screenSize.x, screenSize.y), user.getUnlockedLevel());
						} catch (Exception e1) {
							e1.printStackTrace();
						}
						break;
					case MULTIPLAYER_MENU:
						menuList[menuState.getValue()] =  new MultiplayerMenu(new DynamicBitmap(
									Parameters.bmpBkSubMenu, new Point(0, 0), 0, screenSize.x, screenSize.y));
						break;
					case SHOP_MENU:
						menuList[menuState.getValue()] = new ShopMenu(new DynamicBitmap(
									Parameters.bmpBkSubMenu, new Point(0, 0), 0, screenSize.x, screenSize.y));
						break;
					case SETTING_MENU:
						menuList[menuState.getValue()] = new SettingMenu(new DynamicBitmap(
									Parameters.bmpBkSubMenu, new Point(0, 0), 0, screenSize.x, screenSize.y));
						break;
					case PAUSE_MENU:
						menuList[menuState.getValue()] = new PauseMenu(new DynamicBitmap(
									Parameters.bmpBtnHalfTransparent, new Point(0, 0), 0, screenSize.x, screenSize.y));
						break;
					case MSSG_BOX:
							int tmp;
							tmp = (screenSize.y - Parameters.bmpMssgBox.getHeight()) / 2;
							menuList[menuState.getValue()] = new MssgBox(new DynamicBitmap(Parameters.bmpMssgBox,
									new Point(0, tmp)));
						break;
					case CONGRAT_BOX:
						menuList[menuState.getValue()] = new CongratBox(new DynamicBitmap(
									Parameters.bmpCongrat, new Point(0, 0), 0, screenSize.x, screenSize.y), (MainActivity)App.getMyContext());
						break;
					default:
						break;
				}
		...
		public static void switchSound() {
			...
			if (hasSound == false) {
				GameManager.spBackground = Helper.stopMediaPlayer(GameManager.spBackground, Parameters.dBackgroundSoundtrack);
				GameManager.spVictory = Helper.stopMediaPlayer(GameManager.spVictory, Parameters.dVictorySoundtrack);
				GameManager.spMenu = Helper.stopMediaPlayer(GameManager.spMenu, Parameters.dMenuSoundtrack);
			} else {
		...
		public static void updateContent() throws Exception {
			((LoadMenu)(GameManager.menuList[GameState.LOAD_MENU.getValue()])).SpawnLevel(user.getUnlockedLevel());
		}
		...
		public static void levelUp(int score) throws Exception {
			...
			// TODO: this is replication with the one in the Game constructor, isnt it?
			//gameData = null;
			//gameData = new MapReader(Parameters.dMapID[GameModel.chosenLevel - 1]);
		...
		public static void prepareGame(GameView gameView, Point screenSize, User user) {
